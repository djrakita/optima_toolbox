...
fn main() {
    ...
    // let q = OwnedParryProximaAsProximityQry::new(PairGroupQryArgsParryProxima::new(ParryShapeRep::OBB, true, false, ProximaTermination::MaxError(0.1), ProximityLossFunction::Hinge, 15.0, 0.6));
    // let q = OwnedParryDistanceAsProximityGroupQry::new(ParryDistanceGroupArgs::new(ParryShapeRep::Full, ParryDisMode::ContactDis, true, false, 0.17855));
    let q = OwnedEmptyToProximityQry::new(());
    let db = robot.get_look_at_differentiable_block(ForwardADMulti2::<adfn<16>>::new(), qf, q, None, &init, vec![20], 32, AxisDirection::Z, AxisDirection::X, LookAtTarget::RobotLink(20), 0.09, 0.6, 0.5, 0.0, 1.0, 0.5, 0.1, 0.5, 0.1);
