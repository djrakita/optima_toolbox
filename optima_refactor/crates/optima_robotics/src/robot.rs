...
use optima_3d_spatial::optima_3d_pose::{O3DPose, O3DPoseCategoryIsometry3, AliasO3DPoseCategory};
...
use optima_linalg::{OLinalgCategoryNalgebra, OVec, OVecCategoryVec, AliasOLinalgCategory};
...
pub struct ORobot<T: AD, C: AliasO3DPoseCategory, L: AliasOLinalgCategory> {
...
impl<T: AD, C: AliasO3DPoseCategory, L: AliasOLinalgCategory> ORobot<T, C, L> {
    ...
    pub fn to_other_generic_types<T2: AD, C2: AliasO3DPoseCategory, L2: AliasOLinalgCategory>(&self) -> ORobot<T2, C2, L2> {
    ...
    */
    ...
    pub fn parry_shape_scene_self_query<Q, V: OVec<T>>(&self, state: &V, query: &OwnedPairGroupQry<T, Q>, pair_selector: &OParryPairSelector, freeze: bool) -> <Q::OutputCategory as OPairGroupQryOutputCategoryTrait>::Output<T, C::P<T>>
    ...
    pub fn parry_shape_scene_external_query<Q, V: OVec<T>>(&self, state: &V, scene: &OParryGenericShapeScene<T, C::P<T>>, query: &OwnedPairGroupQry<T, Q>, pair_selector: &OParryPairSelector, freeze: bool) -> <Q::OutputCategory as OPairGroupQryOutputCategoryTrait>::Output<T, C::P<T>>
        where Q: OPairGroupQryTrait<ShapeCategory=ShapeCategoryOParryShape, SelectorType=OParryPairSelector>
    {
        let fk_res = self.forward_kinematics(state, None);
        self.parry_shape_scene_external_query_from_fk_res(&fk_res, scene, query, pair_selector, freeze)
    }
    pub fn parry_shape_scene_external_query_from_fk_res<Q>(&self, fk_res: &FKResult<T, C::P<T>>, scene: &OParryGenericShapeScene<T, C::P<T>>, query: &OwnedPairGroupQry<T, Q>, pair_selector: &OParryPairSelector, freeze: bool) -> <Q::OutputCategory as OPairGroupQryOutputCategoryTrait>::Output<T, C::P<T>>
        where Q: OPairGroupQryTrait<ShapeCategory=ShapeCategoryOParryShape, SelectorType=OParryPairSelector>
    {
...
impl<C: AliasO3DPoseCategory, L: AliasOLinalgCategory> ORobot<f64, C, L> {
    pub fn get_ik_differentiable_block<E, FQ, Q>(&self, derivative_method: E, filter_query: OwnedPairGroupQry<f64, FQ>, distance_query: OwnedPairGroupQry<f64, Q>, constant_selector: Option<OParryPairSelector>, init_state: &[f64], ik_goal_link_idxs: Vec<usize>, linf_dis_cutoff: f64, dis_filter_cutoff: f64, ee_matching_weight: f64, self_collision_avoidance_weight: f64, min_vel_weight: f64, min_acc_weight: f64, min_jerk_weight: f64) -> DifferentiableBlock<DifferentiableFunctionClassIKObjective<C, L, FQ, Q>, E>
        where E: DerivativeMethodTrait,
              FQ: AliasParryGroupFilterQry,
              Q: AliasParryToProximityQry {

        let last_proximity_filter_state: Arc<RwLock<Option<Vec<f64>>>> = Arc::new(RwLock::new(None));
        let filter_output: Arc<RwLock<Option<OParryFilterOutput>>> = Arc::new(RwLock::new(None));

        let f2 = self.get_ik_objective_function(Arc::new(self.to_other_ad_type::<E::T>()), filter_query.clone(), distance_query.clone(), constant_selector.clone(), init_state, ik_goal_link_idxs.clone(), linf_dis_cutoff, dis_filter_cutoff, ee_matching_weight, self_collision_avoidance_weight, min_vel_weight, min_acc_weight, min_jerk_weight, last_proximity_filter_state.clone(), filter_output.clone());
        let f1= self.get_ik_objective_function(Arc::new(self.clone()), filter_query, distance_query, constant_selector, init_state, ik_goal_link_idxs, linf_dis_cutoff, dis_filter_cutoff, ee_matching_weight, self_collision_avoidance_weight, min_vel_weight, min_acc_weight, min_jerk_weight, last_proximity_filter_state.clone(), filter_output.clone());

        DifferentiableBlockIKObjective::new(derivative_method, f1, f2)
    }
    pub fn get_look_at_differentiable_block<E, FQ, Q>(&self, derivative_method: E, filter_query: OwnedPairGroupQry<f64, FQ>, distance_query: OwnedPairGroupQry<f64, Q>, constant_selector: Option<OParryPairSelector>, init_state: &[f64], ik_goal_link_idxs: Vec<usize>, looker_link: usize, looker_forward_axis: AxisDirection, looker_side_axis: AxisDirection, look_at_target: LookAtTarget<f64, O3DVecCategoryArr>, goal_distance_between_looker_and_look_at_target: f64, linf_dis_cutoff: f64, dis_filter_cutoff: f64, ee_matching_weight: f64, self_collision_avoidance_weight: f64, min_vel_weight: f64, min_acc_weight: f64, min_jerk_weight: f64, look_at_weight: f64, roll_prevention_weight: f64, goal_distance_weight: f64) -> DifferentiableBlock<DifferentiableFunctionClassLookAt<C, L, FQ, Q>, E>
        where E: DerivativeMethodTrait,
              FQ: AliasParryGroupFilterQry,
              Q: AliasParryToProximityQry {
        let last_proximity_filter_state: Arc<RwLock<Option<Vec<f64>>>> = Arc::new(RwLock::new(None));
        ...
        let f2 = self.get_look_at_objective_function(Arc::new(self.to_other_ad_type::<E::T>()), filter_query.clone(), distance_query.clone(), constant_selector.clone(), init_state, ik_goal_link_idxs.clone(), looker_link, looker_forward_axis.clone(), looker_side_axis.clone(), look_at_target.clone(), goal_distance_between_looker_and_look_at_target, linf_dis_cutoff, dis_filter_cutoff, ee_matching_weight, self_collision_avoidance_weight, min_vel_weight, min_acc_weight, min_jerk_weight, look_at_weight, roll_prevention_weight, goal_distance_weight, last_proximity_filter_state.clone(), filter_output.clone());
        let f1 = self.get_look_at_objective_function(Arc::new(self.clone()), filter_query, distance_query, constant_selector, init_state, ik_goal_link_idxs, looker_link, looker_forward_axis.clone(), looker_side_axis.clone(), look_at_target.clone(), goal_distance_between_looker_and_look_at_target, linf_dis_cutoff, dis_filter_cutoff, ee_matching_weight, self_collision_avoidance_weight, min_vel_weight, min_acc_weight, min_jerk_weight, look_at_weight, roll_prevention_weight, goal_distance_weight, last_proximity_filter_state.clone(), filter_output.clone());

        DifferentiableBlock::new(derivative_method, f1, f2)
    }
    pub fn get_default_vanilla_ik_differentiable_block<E: DerivativeMethodTrait>(&self, derivative_method: E, init_state: &[f64], ik_goal_link_idxs: Vec<usize>) -> DifferentiableBlock<DifferentiableFunctionClassIKObjective<C, L, EmptyParryFilter, EmptyToParryProximity>, E> {
...
/// Objective Functions
impl<T: AD, C: AliasO3DPoseCategory, L: AliasOLinalgCategory> ORobot<T, C, L > {
    pub fn get_ik_objective_function<T1, FQ, Q>(&self, robot: Arc<ORobot<T1, C, L>>, filter_query: OwnedPairGroupQry<f64, FQ>, distance_query: OwnedPairGroupQry<f64, Q>, constant_selector: Option<OParryPairSelector>, init_state: &[f64], ik_goal_link_idxs: Vec<usize>, linf_dis_cutoff: f64, dis_filter_cutoff: f64, ee_matching_weight: f64, self_collision_avoidance_weight: f64, min_vel_weight: f64, min_acc_weight: f64, min_jerk_weight: f64, last_proximity_filter_state: Arc<RwLock<Option<Vec<f64>>>>, filter_output: Arc<RwLock<Option<OParryFilterOutput>>>) -> DifferentiableFunctionIKObjective<T1, C, L, FQ, Q>
        where T1: AD,
              FQ: AliasParryGroupFilterQry,
              Q: AliasParryToProximityQry
    {
    ...
    pub fn get_look_at_objective_function<'a, T1, FQ, Q>(&'a self, robot: Arc<ORobot<T1, C, L>>, filter_query: OwnedPairGroupQry<f64, FQ>, distance_query: OwnedPairGroupQry<f64, Q>, constant_selector: Option<OParryPairSelector>, init_state: &[f64], ik_goal_link_idxs: Vec<usize>, looker_link: usize, looker_forward_axis: AxisDirection, looker_side_axis: AxisDirection, look_at_target: LookAtTarget<f64, O3DVecCategoryArr>, goal_distance_between_looker_and_look_at_target: f64, linf_dis_cutoff: f64, dis_filter_cutoff: f64, ee_matching_weight: f64, self_collision_avoidance_weight: f64, min_vel_weight: f64, min_acc_weight: f64, min_jerk_weight: f64, look_at_weight: f64, roll_prevention_weight: f64, goal_distance_weight: f64, last_proximity_filter_state: Arc<RwLock<Option<Vec<f64>>>>, filter_output: Arc<RwLock<Option<OParryFilterOutput>>>) -> DifferentiableFunctionLookAt<T1, C, L, FQ, Q>
        where T1: AD,
              FQ: AliasParryGroupFilterQry,
              Q: AliasParryToProximityQry {
        ...
        DifferentiableFunctionLookAt::new(ik_objective, looker_link, looker_forward_axis, looker_side_axis, look_at_target.to_other_ad_type::<T1>(), T1::constant(goal_distance_between_looker_and_look_at_target), T1::constant(look_at_weight), T1::constant(roll_prevention_weight), T1::constant(goal_distance_weight))
    }
}
impl<T: AD, C: AliasO3DPoseCategory, L: AliasOLinalgCategory> AsRobotTrait<T, C, L> for ORobot<T, C, L> {
    #[inline(always)]
    fn as_robot(&self) -> &ORobot<T, C, L> {
        self
    }
}
impl<T: AD, C: AliasO3DPoseCategory, L: AliasOLinalgCategory> Debug for ORobot<T, C, L> {
...
impl<T: AD, C: AliasO3DPoseCategory, L: AliasOLinalgCategory> ShapeSceneTrait<T, C::P<T>> for ORobot<T, C, L> {
...
pub struct ORobotADConverter<C: AliasO3DPoseCategory, L: AliasOLinalgCategory>(PhantomData<(C, L)>);
impl<C: AliasO3DPoseCategory, L: AliasOLinalgCategory> ADConvertableTrait for ORobotADConverter<C, L> {
    type ConvertableType<T: AD> = ORobot<T, C, L>;

    fn convert_to_other_ad_type<T1: AD, T2: AD>(input: &Self::ConvertableType<T1>) -> Self::ConvertableType<T2> {
        input.to_other_ad_type::<T2>()
    }
}
...
pub enum IKGoalMode<T: AD, C: AliasO3DPoseCategory> {
    Absolute,
