...
use optima_optimization::{GenericOptimizerTrait, OptimizerOutputTrait};
use optima_optimization::optimization_engine::SimpleOpEnEngineGenericOptimizer;
...
fn main() {
    ...
    let s = SimpleOpEnEngineGenericOptimizer::new(robot.get_dof_lower_bounds(), robot.get_dof_upper_bounds(), 0.0001);
    let init = robot.sample_pseudorandom_state();
    let res = s.optimize(&diff_block, &init);
    println!("{:?}", res.x_star());
    println!("{:?}", res.f_star());
